...
router.get("/banners", async (_req, res): Promise<void> => {
  ...
  res.json(banners.map(b => ({ id: b.id, title: b.title, subtitle: b.subtitle, image: b.image, link: b.link, isActive: b.isActive, sortOrder: b.sortOrder, createdAt: b.createdAt })));
});
...
router.post("/banners", requireAdmin, async (req, res): Promise<void> => {
  ...
  res.status(201).json({ id: banner.id, title: banner.title, subtitle: banner.subtitle, image: banner.image, link: banner.link, isActive: banner.isActive, sortOrder: banner.sortOrder, createdAt: banner.createdAt });
});
...
router.put("/banners/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  res.json({ id: banner.id, title: banner.title, subtitle: banner.subtitle, image: banner.image, link: banner.link, isActive: banner.isActive, sortOrder: banner.sortOrder, createdAt: banner.createdAt });
});
