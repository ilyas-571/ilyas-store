...
router.get("/categories", async (_req, res): Promise<void> => {
  ...
  const result = await Promise.all(categories.map(async (cat) => {
    const [countRow] = await db.select({ count: sql<number>`count(*)::int` }).from(productsTable).where(eq(productsTable.categoryId, cat.id));
    return { id: cat.id, name: cat.name, image: cat.image, parentId: cat.parentId, sortOrder: cat.sortOrder, productCount: countRow?.count ?? 0, createdAt: cat.createdAt };
  }));
  res.json(result);
});
...
router.post("/categories", requireAdmin, async (req, res): Promise<void> => {
  ...
  res.status(201).json({ id: cat.id, name: cat.name, image: cat.image, parentId: cat.parentId, sortOrder: cat.sortOrder, productCount: 0, createdAt: cat.createdAt });
});
...
router.put("/categories/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (req.body?.parentId !== undefined) updateData.parentId = Number.isInteger(req.body.parentId) ? req.body.parentId : null;
  ...
  const [countRow] = await db.select({ count: sql<number>`count(*)::int` }).from(productsTable).where(eq(productsTable.categoryId, cat.id));
  res.json({ id: cat.id, name: cat.name, image: cat.image, parentId: cat.parentId, sortOrder: cat.sortOrder, productCount: countRow?.count ?? 0, createdAt: cat.createdAt });
});

router.put("/categories/sort", requireAdmin, async (req, res): Promise<void> => {
  const items = Array.isArray(req.body?.items) ? req.body.items : [];
  for (const item of items) {
    if (!Number.isInteger(item?.id)) continue;
    await db.update(categoriesTable).set({ sortOrder: Number(item?.sortOrder) || 0 }).where(eq(categoriesTable.id, item.id));
  }
