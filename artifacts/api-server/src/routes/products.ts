...
async function buildProductResponse(
  ...
  const effectiveStock =
    variantStock >= 0 ? variantStock : product.stock;
  return {
    sku: product.sku,
    slug: product.slug,
    id: product.id,
...
router.get("/products", async (req: TenantRequest, res): Promise<void> => {
  ...
  if (category) {
    ...
    if (!isNaN(parsedId)) {
      ...
      if (cats.length > 0)
        conditions.push(inArray(productsTable.categoryId, cats.map((c) => c.id)));
  ...
  if (minPrice != null)
    conditions.push(
      gte(sql`${productsTable.basePrice}::numeric`, minPrice)
    );
  if (maxPrice != null)
    conditions.push(
      lte(sql`${productsTable.basePrice}::numeric`, maxPrice)
    );
  if (featured != null)
    conditions.push(eq(productsTable.isFeatured, featured));
  ...
  const products = await Promise.all(
    paginated.map(buildProductResponse)
  );
...
router.get("/filters", async (req: TenantRequest, res): Promise<void> => {
  ...
  try {
    ...
    const brands = await db
      ...
      .from(productsTable)
      .where(eq(productsTable.storeId, req.storeId));
    const categories = await db
      .select({ name: categoriesTable.name, id: categoriesTable.id })
      .from(categoriesTable)
      .where(eq(categoriesTable.storeId, req.storeId));
    ...
    res
      .status(500)
      .json({ error: err.message || "Failed to fetch filters" });
  }
});

router.get(
  "/:id/recommendations",
  async (req: TenantRequest, res): Promise<void> => {
    ...
    try {
      ...
      const results = await Promise.all(
        recommendations.map(buildProductResponse)
      );
      res.json({ recommendations: results });
    } catch (err: any) {
      res
        .status(500)
        .json({ error: err.message || "Failed to fetch recommendations" });
    }
  }
);
          ...
          .values({
            ...
            costPrice:
              parsed.data.costPrice != null
                ...
                : null,
            compareAtPrice:
              parsed.data.compareAtPrice != null
                ? String(parsed.data.compareAtPrice)
                : null,
            description: parsed.data.description ?? null,
            images: parsed.data.images ?? [],
            stock:
              variantStock >= 0 ? variantStock : parsed.data.stock,
...
router.get("/products/featured", async (_req, res): Promise<void> => {
  ...
  const result = await Promise.all(
    products.map(buildProductResponse)
  );
  res.json(result);
});
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      updateData.stock = parsed.data.stock;
    if (parsed.data.isFeatured != null)
      updateData.isFeatured = parsed.data.isFeatured;
...
router.post(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to import product" });
    }
  }
);
