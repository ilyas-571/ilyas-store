...
export default function LoginPage() {
  ...
  if (user) {
    navigate(isAdminRole(user.role as string) ? "/admin" : "/");
    return null;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        ...
        navigate(isAdminRole((res as any).data?.user?.role) ? "/admin" : "/");
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-foreground flex-col justify-between p-12">
        ...
        <div>
          <blockquote className="font-serif text-2xl text-background/80 leading-relaxed mb-6">
            "Luxury is not about price. It is about the weight of what endures."
          </blockquote>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          <div className="mb-10">
            <p className="text-xs tracking-[0.3em] uppercase text-muted-foreground font-sans mb-2">Welcome back</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" className="w-full h-11 font-sans tracking-wide" disabled={loginMutation.isPending}>
          ...
          <p className="mt-8 text-center text-sm text-muted-foreground font-sans">
            New to Ilyas Store?{" "}
            <Link href="/register" className="text-foreground hover:underline font-medium">Create account</Link>
          </p>
        </div>
      </div>
    </div>
