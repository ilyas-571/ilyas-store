...
export default function AdsAnalytics() {
  ...
  useEffect(() => {
    fetch("/api/ads/analytics", { headers: headers() })
      .then((r) => r.json())
      .then(setData);
    fetch("/api/ads/campaigns", { headers: headers() })
      .then((r) => r.json())
      .then(setCampaigns);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <MetricCard
          label="Ad Spend"
          value={`PKR ${data.totalSpend.toLocaleString()}`}
          icon={DollarSign}
          color="bg-amber-500"
        />
        <MetricCard
              ...
              <div key={p.id} className="flex items-center gap-3 px-5 py-3">
                <div className="h-9 w-9 bg-muted rounded-md shrink-0 overflow-hidden">
                  {p.image ? (
                    ...
                    <Package className="h-4 w-4 m-auto mt-2.5 text-muted-foreground/30" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground font-sans">
                    In {p.campaigns} campaign{p.campaigns !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
            ))}
      ...
      {campaigns.length === 0 && (
        <div className="border border-dashed border-border rounded-lg p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground font-sans">
            Create campaigns to start tracking analytics
          </p>
        </div>
      )}
    </div>
